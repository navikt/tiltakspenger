...
- **Sjekk at et repo du lenker til ikke er arkivert.** En fil i et arkivert repo svarer fortsatt på URL-en. Sjekk `archived` og `default_branch` i `https://api.github.com/repos/navikt/<repo>` før du lenker fra KDoc eller dokumentasjon.
...
- **Labelen `agent`** betyr «egnet for en KI-agent»: avgrensede tekniske oppgaver med tydelig mål (migreringer, CI, tester, feil med kjent årsak). Bruk den ikke på issues med `avklaring`- eller `pvk`-label, produktønsker uten beskrivelse, eller oppgaver som krever menneskelige avgjørelser eller tilganger.
- **Repoene og issuene er offentlige.** I issues og dokumentasjon: ingen fødselsnumre (heller ikke syntetiske; bruk sak-ID), ingen personnavn på kollegaer (bruk team eller rolle), ingen vurderinger av kapasitet eller kompetanse. Interne lenker til Confluence, Slack og Jira kan stå.
...
- **GitHub Actions-workflows skal være så like som mulig på tvers av repoene.** Når du endrer CI i ett repo, vurder om de andre repoene bør endres tilsvarende, slik at oppsettet konvergerer i stedet for å sprike. Konkret:
    ...
    - **Delte reusable workflows** bor i det private repoet [`navikt/tiltakspenger-workflows`](https://github.com/navikt/tiltakspenger-workflows) (`.github/workflows/`, se README-en der) og kalles fra repoene med `workflow_call` og `uses: navikt/tiltakspenger-workflows/.github/workflows/<navn>.yml@main` — les README-en der (caller-eksempel, secrets-/permissions-konvensjoner) før du endrer eller dupliserer CI-logikk, og foretrekk å utvide en delt workflow fremfor å kopiere den inn i et enkelt-repo. Malene for `zizmor.yml`/`dependabot.yml` bor også der (`.github/maler/`) — lint-workflowen curler dem derfra uten token.
    ...
    - **Styr repo-avvik med en input, ikke med reponavn.** Et unntak for ett repo er en opt-out-input med default som beholder dagens oppførsel, ikke `if: github.repository != …` i den delte workflowen.
    - **Deklarer nye inputs og secrets i den delte workflowen før callerne bruker dem.** En input eller secret som ikke er deklarert der gir `startup_failure`.
    - **Callere kan ikke droppe `id-token: write` selv om jobben skippes.** GitHub sjekker den kalte workflowens permissions mot callerens før `if`-betingelsene evalueres.
    - **Send secrets eksplisitt.** Bruk `secrets: inherit` bare mot workflows som ikke deklarerer secretsene sine (`navikt/tms-deploy`), merket `# zizmor: ignore[secrets-inherit]` på `uses:`-linja. Lint validerer ikke mot en ekstern callee.
    - **En ny gate i en delt workflow er ikke verifisert i et repo før en vanlig PR har truffet den.** Dependabot-PR-er bygges av auto-merge-workflowen og hopper over gaten.
    - **Alert-manifester som deployes med variabler går gjennom handlebars.** Skriv Prometheus-maler i annotasjoner som `'\{{ $labels.app }}'` i enkle anførselstegn, også i kommentarer; et uescapet `{{ $labels.app }}` blir tom streng. Sjekk `PRINT_PAYLOAD` i deploy-loggen.
    - **Vær varsom med workflow-endringer i `tiltakspenger-iac`.** Deploy-workflowene lister sin egen fil i `paths`, så en ren workflow-endring deployer alerts, kafka og terraform til dev og prod ved merge, uten test-gate foran.
    - **Dependabot ser ikke Chainguard-bildene fra Navs speil** (`cgr-nav/pull-through`). Bruk flytende tagg med en kommentar som sier hvorfor; digest-lås bare for bilder Dependabot kan lese.
...
- `gcloud`-tokenet utløper og må fornyes interaktivt, så `kubectl` kan falle bort midt i en økt. Loki/Tempo/Mimir over API virker uansett — bruk dem til logg- og trafikkoppfølging etter deploy. Feilen ser slik ut: `print credential failed … Reauthentication failed. cannot prompt during non-interactive execution`; da må utvikleren kjøre `nais login` på nytt i egen terminal, ikke `gcloud auth login`.
- **Bruk `start` og `end` i Loki-spørringer**, i nanosekunder. `since` respekteres ikke alltid, og uten eksplisitt tidsrom får du default-vinduet på siste time.
- **Send Tempo-søk som GET.** Bruk `curl -G` med `--data-urlencode`, eller legg parametrene i URL-en; ellers ignoreres filteret, og svaret er ufiltrerte, nylige traces.
- **`query_range` med `step` lik hele vinduet gir tomt resultat** uten feilmelding, selv når linjer ble prosessert. Bruk `step=86400` eller mindre.
- **Tellere nullstilles når prosessen starter på nytt, altså ved hver deploy.** `sum(teller_total)` viser bare det dagens pods har samlet; bruk `sum(increase(teller_total[vindu]))`. For en serie som først oppstår inne i vinduet får `increase()` ikke med det første hoppet; sjekk da råverdiene med `max_over_time`, og om serien har målepunkter i det hele tatt.
- **Deployede alert-regler kan leses fra clusteret.** `kubectl --context prod-gcp -n tpts get prometheusrule <app>-alerts -o jsonpath=…` viser reglene slik de ble stående; `PRINT_PAYLOAD` viser bare det som ble sendt.
